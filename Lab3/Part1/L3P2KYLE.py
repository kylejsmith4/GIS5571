...
# Get the map named 'L2'
l2_maps = aprx.listMaps("L2")
if len(l2_maps) == 0:
    # Create a new map named 'L2' and add it to the project
    l2_map = arcpy.mp.Map("L2")
    aprx.addMap(l2_map)
    aprx.save()
    print("Created new map 'L2' and added it to the project.")
else:
    l2_map = l2_maps[0]
    print("Map 'L2' found and will be used.")
...
l2_map.addDataFromPath(dory_farm_start)
...
l2_map.addDataFromPath(north_picnic_end)
...
# Find the land cover raster file
land_raster_files = [f for f in os.listdir(land_extract_dir) if f.endswith('.tif')]
if land_raster_files:
    land_raster_path = os.path.join(land_extract_dir, land_raster_files[0])
else:
    raise FileNotFoundError("No land cover raster found in the extracted data.")
...
l2_map.addDataFromPath(farm_fields_reclass_path)
...
l2_map.addDataFromPath(hydro_clipped_path)
l2_map.addDataFromPath(bridge_clipped_path)
...
l2_map.addDataFromPath(water_reclass_path)
...
l2_map.addDataFromPath(dem_clipped_path)
...
l2_map.addDataFromPath(slope_raster_path)
...
l2_map.addDataFromPath(slope_reclass_path)
...
l2_map.addDataFromPath(cost_surface_weighted_path)
...
if arcpy.Exists(least_cost_path_raster_path):
    try:
        l2_map.addDataFromPath(least_cost_path_raster_path)
...
weighting_schemes = [
    {'name': 'Equal_Weights', 'slope': 0.33, 'farm': 0.33, 'water': 0.34},
    {'name': 'Slope_Emphasis', 'slope': 0.6, 'farm': 0.2, 'water': 0.2},
    {'name': 'Avoid_Farm_Fields', 'slope': 0.2, 'farm': 0.6, 'water': 0.2}
]

# Paths to reclassified rasters (ensure these are defined earlier in your script)
slope_reclass_path = os.path.join(Dory_folder, "Slope_Reclass.tif")
farm_fields_reclass_path = os.path.join(Dory_folder, "Farm_Reclassified.tif")
water_reclass_path = os.path.join(Dory_folder, "Water_Reclassified.tif")
...
for scheme in weighting_schemes:
    cost_surface_path = os.path.join(Dory_folder, f"Cost_Surface_{scheme['name']}.tif")
    if not arcpy.Exists(cost_surface_path):
        # Create Raster objects from file paths
        slope_raster = Raster(slope_reclass_path)
        farm_raster = Raster(farm_fields_reclass_path)
        water_raster = Raster(water_reclass_path)

        # Calculate the weighted cost surface
        cost_surface = (scheme['slope'] * slope_raster +
                        scheme['farm'] * farm_raster +
                        scheme['water'] * water_raster)
        cost_surface.save(cost_surface_path)
        print(f"Saved cost surface: {cost_surface_path}")
    else:
        print(f"Cost surface already exists: {cost_surface_path}"
              
# ---

              
# Paths to start and end points (ensure these are defined earlier in your script)
...
# Ensure that the point feature classes exist
for feature_path in [dory_farm_start, north_picnic_end]:
    if not arcpy.Exists(feature_path):
        raise FileNotFoundError(f"Feature class not found: {feature_path}")

# Loop through each weighting scheme to compute cost distance and least-cost paths
for scheme in weighting_schemes:
    cost_surface_path = os.path.join(Dory_folder, f"Cost_Surface_{scheme['name']}.tif")
    cost_distance_raster_path = os.path.join(Dory_folder, f"Cost_Distance_{scheme['name']}.tif")
    backlink_raster_path = os.path.join(Dory_folder, f"Backlink_{scheme['name']}.tif")
    least_cost_path_raster_path = os.path.join(Dory_folder, f"Least_Cost_Path_{scheme['name']}.tif")
    ...
    if not arcpy.Exists(cost_distance_raster_path):
        try:
            ...
            print("Error in CostDistance:")
            print(arcpy.GetMessages())
            continue  # Skip to the next scheme if error occurs
...
# Define the least-cost path polylines and their corresponding point shapefiles
paths = [
    {
        'polyline': "Least_Cost_Path_Equal_Weights.shp",
        'points': "Path_Points_Equal_Weights.shp"
    },
    {
        'polyline': "Least_Cost_Path_Slope_Emphasis.shp",
        'points': "Path_Points_Slope_Emphasis.shp"
    },
    {
        'polyline': "Least_Cost_Path_Avoid_Farm_Fields.shp",
        'points': "Path_Points_Avoid_Farm_Fields.shp"
    }
]

# Calculate Slope Statistics
Slope_stats_csv = os.path.join(Dory_folder, "Slope_Statistics.csv")
with open(Slope_stats_csv, 'w', newline='') as csvfile:
    fieldnames = ['Path', 'Average_Slope', 'Maximum_Slope', 'Minimum_Slope']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    
    for path in paths:
        point_shp = os.path.join(Dory_folder, path['points'])
        polyline_name = path['polyline']
        
        # Check if the point shapefile exists
        if not arcpy.Exists(point_shp):
            print(f"Point shapefile not found: {point_shp}. Skipping slope statistics.")
            continue
        
        # Perform summary statistics for slope
        temp_table = "in_memory\\temp_slope_stats"
        try:
            arcpy.Statistics_analysis(
                in_table=point_shp,
                out_table=temp_table,
                statistics_fields=[["SlopeValue", "MEAN"], ["SlopeValue", "MAX"], ["SlopeValue", "MIN"]],
                case_field=""
            )
            
            # Read the statistics
            with arcpy.da.SearchCursor(temp_table, ["MEAN_SlopeValue", "MAX_SlopeValue", "MIN_SlopeValue"]) as cursor:
                for row in cursor:
                    writer.writerow({
                        'Path': polyline_name,
                        'Average_Slope': f"{row[0]:.2f}",
                        'Maximum_Slope': f"{row[1]:.2f}",
                        'Minimum_Slope': f"{row[2]:.2f}"
                    })
            print(f"Calculated slope statistics for {polyline_name} and saved to CSV.")
        except arcpy.ExecuteError:
            print(f"Error calculating slope statistics for {polyline_name}:")
            print(arcpy.GetMessages())

# Calculate Cost Distance Statistics
Cost_distance_stats_csv = os.path.join(Dory_folder, "CostDistance_Statistics.csv")
with open(Cost_distance_stats_csv, 'w', newline='') as csvfile:
    fieldnames = ['Path', 'Average_CostDistance', 'Maximum_CostDistance', 'Minimum_CostDistance']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    ...
    for path in paths:
        ...
        # Check if the point shapefile exists
        if not arcpy.Exists(point_shp):
            print(f"Point shapefile not found: {point_shp}. Skipping cost distance statistics.")
            continue
        
        # Perform summary statistics for cost distance
        temp_table = "in_memory\\temp_cost_distance_stats"
        try:
            arcpy.Statistics_analysis(
                in_table=point_shp,
                out_table=temp_table,
                statistics_fields=[["CostDistance", "MEAN"], ["CostDistance", "MAX"], ["CostDistance", "MIN"]],
                case_field=""
            )
            
            # Read the statistics
            with arcpy.da.SearchCursor(temp_table, ["MEAN_CostDistance", "MAX_CostDistance", "MIN_CostDistance"]) as cursor:
                for row in cursor:
                    writer.writerow({
                        'Path': polyline_name,
                        'Average_CostDistance': f"{row[0]:.2f}",
                        'Maximum_CostDistance': f"{row[1]:.2f}",
                        'Minimum_CostDistance': f"{row[2]:.2f}"
                    })
            print(f"Calculated cost distance statistics for {polyline_name} and saved to CSV.")
        except arcpy.ExecuteError:
            print(f"Error calculating cost distance statistics for {polyline_name}:")
            print(arcpy.GetMessages())

# Calculate Land Cover Statistics (Percentage Distribution)
Land_cover_stats_csv = os.path.join(Dory_folder, "LandCover_Statistics.csv")
with open(Land_cover_stats_csv, 'w', newline='') as csvfile:
    fieldnames = ['Path', 'LandCoverType', 'Percentage']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    ...
    for path in paths:
        ...
        # Check if the point shapefile exists
        if not arcpy.Exists(point_shp):
            print(f"Point shapefile not found: {point_shp}. Skipping land cover statistics.")
            continue
        
        # Perform frequency analysis for land cover
        temp_table = "in_memory\\temp_land_cover_stats"
        try:
            arcpy.Statistics_analysis(
                in_table=point_shp,
                out_table=temp_table,
                statistics_fields=[],  # No statistics fields, only case_field
                case_field=["LandCoverValue"]
            )
            ...
            with arcpy.da.SearchCursor(temp_table, ["LandCoverValue", "FREQUENCY"]) as cursor:
                for row in cursor:
                    land_cover = row[0]
                    count = row[1]
                    land_cover_counts[land_cover] = count
                    total_count += count
            
            # Write to CSV
            for land_cover, count in land_cover_counts.items():
            ...
            print(f"Calculated land cover statistics for {polyline_name} and saved to CSV.")
        except arcpy.ExecuteError:
            print(f"Error calculating land cover statistics for {polyline_name}:")
            print(arcpy.GetMessages())